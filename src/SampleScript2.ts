...
import type { Calculation, MonthlyHolidayAccrual, HolidayPay, UserDefinedRow } from "@salaxy/core";
import type { RowTypeScripts } from "../@salaxy/expressions";

export class hallitus_2 implements RowTypeScripts {
  public price(): number {
...
export class ylityo_vrk_50 implements RowTypeScripts {
  /**
  ...
  private getPricingPeriodStartDate(workStartDate: string | null | undefined): string {
...
export class poissaolopalkallinen implements RowTypeScripts {
  ...
  public price(): number {
    ...
    if (dividerConfig) {
      return Helpers.round(monthlyPrice / (dividerConfig.data));
    }
    else {
      return Helpers.round(monthlyPrice / 158);
    }
  }
...
export class SampleScript2 {
  ...
  public payStats_result(): PayStatsCalculationResultValue {
    ...
    payStatsResult.result = Helpers.round(payStatsResult.salary / (payStatsResult.workingTime || 1));
